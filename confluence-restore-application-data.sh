...
BACKUP_PATH="/srv/confluence-application-data/backups/"
RESTORE_PATH="/var/atlassian/application-data/confluence/"
BACKUP_PREFIX="confluence-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$CONFLUENCE_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$CONFLUENCE_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
