#!/usr/bin/env bash
# confluence-restore-application-data.sh [backup-file-name]
#
# Replaces Confluence's application data with one of the archives the backups
# service wrote.
#
#   ./confluence-restore-application-data.sh               list and ask
#   ./confluence-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Confluence." >&2
  exit 1
fi
