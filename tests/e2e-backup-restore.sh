#!/bin/bash
# End-to-end tests for the confluence-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-confluence}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-confluence-traefik-letsencrypt-docker-compose.yml}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./confluence-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./confluence-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./confluence-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./confluence-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
