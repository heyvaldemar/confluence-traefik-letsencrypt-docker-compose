#!/usr/bin/env bash
# confluence-restore-database.sh [backup-file-name]
#
# Replaces the Confluence database with one of the dumps the backups service wrote.
#
#   ./confluence-restore-database.sh               list and ask
#   ./confluence-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than confluence.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-confluence}"
APP_SERVICE="confluence"
...
DB_NAME="$(env_of CONFLUENCE_DB_NAME)"; DB_USER="$(env_of CONFLUENCE_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Confluence." >&2
  exit 1
fi
