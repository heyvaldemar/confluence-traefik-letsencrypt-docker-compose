...
**Restore** with the interactive scripts (`chmod +x *.sh` once): `./confluence-restore-database.sh`, then `./confluence-restore-application-data.sh`.

## Testing

The [Deployment Verification](https://github.com/heyvaldemar/confluence-traefik-letsencrypt-docker-compose/actions/workflows/deployment-verification.yml?query=branch%3Amain) workflow runs on every push, pull request, and every Monday at 06:00 UTC:
...
3. **Pin freshness** (weekly/manual) — digest drift, Confluence Docker Hub tag lag, Traefik release lag.
